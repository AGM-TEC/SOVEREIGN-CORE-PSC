#!/bin/bash
echo "🛡️ DÉPLOIEMENT SOVEREIGN WORKSTATION v4.0 PRO"
pkill -9 java 2>/dev/null
sleep 1
rm -rf build/classes build/temp_sources
mkdir -p build/classes build/temp_sources
CP=$(echo libs/*.jar | tr ' ' ':')

cat << 'INNER_EOF' > build/temp_sources/Main.kt
...
kotlinc build/temp_sources/Main.kt -cp "$CP" -include-runtime -d build/SovereignCore.jar
java -Xmx256M -cp "build/SovereignCore.jar:$CP" com.fardc.sigint.core.MainKt
