cat << 'EOF' > scripts/build_release.sh
#!/bin/bash
echo "🛡️ SOVEREIGN-CORE-PSC | Préparation de la Release v2.4.0 (COMBAT-READY)"
echo "------------------------------------------------------"

# 1. Dossiers de sortie
mkdir -p build/libs
mkdir -p dist/

# 2. Construction du Classpath détaillé
CP="libs/javalin-5.6.3.jar:libs/slf4j-api-2.0.7.jar:libs/slf4j-simple-2.0.7.jar:libs/jackson-databind-2.15.2.jar:libs/jackson-core-2.15.2.jar:libs/jackson-annotations-2.15.2.jar:libs/jetty-server-11.0.15.jar:libs/jetty-util-11.0.15.jar:libs/jetty-http-11.0.15.jar:libs/jetty-io-11.0.15.jar"

echo "⚙️  Compilation du Noyau et du CombatModeHandler..."
# Le joker *.kt inclut désormais CombatModeHandler.kt et les modules SIGINT
kotlinc src/main/kotlin/com/fardc/sigint/core/*.kt \
-cp "$CP" \
-include-runtime -d build/libs/sigint-core-all.jar

if [ $? -eq 0 ]; then
    echo "✅ COMPILATION RÉUSSIE."
    cp build/libs/sigint-core-all.jar dist/
    echo "🚀 RELEASE v2.4.0 GÉNÉRÉE : dist/sigint-core-all.jar"
else
    echo "❌ ÉCHEC DE LA COMPILATION. Vérifiez MPesaCommander ou CombatModeHandler."
    exit 1
fi
EOF
chmod +x scripts/build_release.sh
